...
let rijen;
try {
  const res = await fetch(`${url}/rest/v1/claims_public?select=ext_ref,text,ring,dated,dated_precisie,dated_soort,bron_url,bron_naam&order=ext_ref`, {
...
const projectie = {
  ...
  claims: wp.map(r => ({ ext_ref: r.ext_ref, text: r.text, dated: r.dated, dated_precisie: r.dated_precisie, dated_soort: r.dated_soort, bron_url: r.bron_url, bron_naam: r.bron_naam, ring: r.ring }))
};
