...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // A host that re-renders the light DOM (e.g. a React-backed DC) can
      // reconcile data-deck-active off the slides after _applyIndex set it,
      // leaving every slide hidden. Re-apply whenever the attribute goes
      // missing. Self-stable: the re-apply restores the attribute, so the
      // next callback sees it present and does nothing.
      if (!this._activeGuard && typeof MutationObserver === 'function') {
        const reapply = () => {
          if (!this._slides || !this._slides.length) return;
          // Self-stable: bail out once a slide carries the attribute again.
          if (this.querySelector(':scope > [data-deck-active]')) return;
          this._applyIndex({ showOverlay: false, broadcast: false, reason: 'init' });
        };
        // data-deck-active lives on the SECTION CHILDREN, so the subtree must be
        // observed — watching the host alone never sees the reconciliation.
        this._activeGuard = new MutationObserver(reapply);
        this._activeGuard.observe(this, {
          childList: true,
          subtree: true,
          attributes: true,
          attributeFilter: ['data-deck-active'],
        });
        // Cover the case where the attribute was never set at all: no mutation
        // exists to observe, so apply once directly after this mount.
